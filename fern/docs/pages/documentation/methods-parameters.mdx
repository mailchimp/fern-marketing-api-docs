...
<Note>
**Note**: You won’t need to worry too much about these implementation details if you’re using the [official client libraries](https://mailchimp.com/developer/tools/).
</Note>
...
<Note>
**Note**: We refer to some endpoints by name and others by URI. For example, you can find a collection of reports at the [Reports endpoint](/marketing/api), or individual reports at the [/reports/{campaign_id} endpoint](/marketing/api).
</Note>
...
However, if you wanted to return just the name of a specific audience, you would do so with the [`/lists/{list_id} endpoint`](/marketing/api) and the audience name field should be referred to as `name`, like this:
