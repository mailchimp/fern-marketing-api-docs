...
While the Export API continues to be supported, if you’re new to Mailchimp, we recommend using the Batch endpoint for these purposes instead. To get started, see the [Run Async Requests with the Batch Endpoint](/marketing/api-concepts/batch-operations) guide.
...
Unlike the Marketing API, the Export API does not use [HTTP basic authentication](/marketing/api-concepts/authentication#authenticate-with-an-api-key-or-oauth-2-token). Instead, you’ll authenticate requests by providing your account’s API key as the value for the `apikey` parameter.
...
<Note>
**Note**: To receive and process export data in a more traditional fashion, take a look at [the Batch endpoint](/marketing/api-concepts/batch-operations).
</Note>
