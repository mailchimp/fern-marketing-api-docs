...
To start, you’ll need to navigate to the [**Settings**](https://mandrillapp.com//settings) page and set up an API key.  Find out more about setting up an API key at [Transactional API Quick Start](https://mailchimp.com/developer/transactional/guides/quick-start/#generate-your-api-key).

All API URLs listed in this documentation for SMS messages are referring to  the Mailchimp Transactional API, v1.1 and the [send-sms endpoint](https://mailchimp.com/developer/transactional/api/messages/send-sms-message/).
...
To assign tags or metadata to an SMS send, use the `tag` and `metadata` parameter of the [send-sms endpoint](https://mailchimp.com/developer/transactional/api/messages/send-sms-message/).
...
To find out more about subaccounts, including how to create one, see [**Subaccounts**](https://mailchimp.com/developer/transactional/docs/subaccounts/#create-a-subaccount).

To assign a subaccount to an SMS send, use the `subaccount` parameter of the [send-sms endpoint](https://mailchimp.com/developer/transactional/api/messages/send-sms-message/).
...
Webhooks allow your application to receive information about SMS events as they occur and respond in a way that you define, similar to how [webhooks can be used for email events](https://mailchimp.com/developer/transactional/docs/webhooks/).

**Add a new webhook**

To set up a webhook, follow the steps located [here](https://mailchimp.com/developer/transactional/docs/webhooks/#add-a-new-webhook), but choose the events under the SMS section that you would like the webhook to listen for.
...
For SMS events, the response will still follow a similar format as described [here](https://mailchimp.com/developer/transactional/docs/webhooks/#detailed-webhook-format-responses), with some differences. There will be a “channel” parameter that will help differentiate between a SMS message and an email message.
