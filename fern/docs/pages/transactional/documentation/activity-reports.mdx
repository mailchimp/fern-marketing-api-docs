...
Overall statistics—including total deliveries, bounces and rejections, unique opens and clicks, and total opens and clicks—are stored indefinitely. You can view these overall stats on the [**Dashboard**](https://mandrillapp.com/), or using the [senders](/transactional/api), [tags](/transactional/api), and [templates](/transactional/api) API calls.
...
You can turn open tracking on or off via the [**Sending Defaults**](https://mandrillapp.com/settings/sending-options) page. To override these global settings on a per-message basis, use the [`track_opens`](/transactional/api) parameter (API) or the [`X-MC-Track`](/transactional/docs/smtp-integration/#x-mc-track) header (SMTP). 
...
Click tracking is enabled by default for both HTML and text emails. You can adjust this for one or both types of messages in your [**Sending Defaults**](https://mandrillapp.com/settings/sending-options), or you can override those global settings on a per-message basis with the [`track_clicks`](/transactional/api) parameter (API) or the [`X-MC-Track`](/transactional/docs/smtp-integration/#x-mc-track) header (SMTP).
...
Once the domain is set up in your account, you can enable it as your custom tracking domain in [**Sending Defaults**](https://mandrillapp.com/settings/sending-options). If you want to override those global settings on a per-message basis, use the [`tracking_domain`](/transactional/api) parameter (API) or [`X-MC-TrackingDomain`](/transactional/docs/smtp-integration/#x-mc-trackingdomain) header (SMTP).
...
With the API, use the [`messages/send`](/transactional/api) or [`messages/send-template`](/transactional/api) endpoint and  set the following within the message parameter:
...
Mailchimp Transactional automatically indexes a number of fields that you can use to search your sending and find relevant trends. Search is available in the app or [via the API](/transactional/api).
...
You can search your indexed metadata fields in **Outbound Activity** or via the API’s [`messages/search`](/transactional/api) call. To search for an indexed field, prefix the metadata field name with u_ in your search query:
...
You can export activity for your account via the [Outbound Activity](https://mandrillapp.com/activity) page or using the Transactional API [`exports/activity`](/transactional/api) call.
