...
You can filter both tags and metadata via the **View Stats By **drop-down menu on your [**Dashboard**](https://mandrillapp.com/), or by using the [`/messages/search`](/transactional/api/messages/search) and [`/messages/search-time-series`](/transactional/api/messages/search-time-series) API calls. Tag-based stats are kept indefinitely and are viewable for the lifetime of the account; metadata stats in **Outbound Activity** are only viewable for 30 days.
...
When you send an email, you can add tags to your [API call](/transactional/api/messages/send). For example, to add `order-confirmation` and `order-shipped` to your list of tags, add the array:
...
You can delete a tag manually in the [**Tags**](https://mandrillapp.com/tags) section of the app; alternatively, you can use the [`tags/delete`](/transactional/api/tags/delete) endpoint in the Transactional API. 
...
Via the API, include metadata as part of the message parameter with the [`messages/send`](/transactional/api/messages/send) or [`messages/send-template`](/transactional/api/messages/send-template) endpoints. For example, a payload with metadata included might look like this:
