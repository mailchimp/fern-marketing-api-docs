...
1. Add your custom code to a template via the [Transactional API](/transactional/api)
...
Templates have several default options, including a subject line, sender, HTML content, and text content. None of these fields are required when creating or changing a template, but if you’re planning to use the [`messages/send-template`](https://mailchimp.com/developer/api/transactional/messages/send-using-message-template/) endpoint, be sure to have either HTML or text content specified, since it will form the body of your email.

<Note>
**Note**: In addition to [`messages/send-template`](/transactional/api) or specifying a template through SMTP headers, you can also apply templates using the [rules engine](/transactional/docs/rules-engine) or when A/B split testing.
</Note>
...
For example, to include an order ID in a subject line, first add the merge tag in the value for the `subject` parameter in your [API request](/transactional/api):
...
With the [`messages/send`](/transactional/api) endpoint in the Transactional API, use the `template_content` parameter to distinguish any regions to be replaced and the content to be injected into the template’s HTML. Your call to the API could look something like this:
