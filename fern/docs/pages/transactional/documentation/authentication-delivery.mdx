...
You can also add a sender domain via the [Transactional API](/transactional/api).
...
Add this to your domain’s TXT records, and then click **Test DNS Settings** to check if the record is available. Just like with DKIM, your domain’s TTL may require several minutes’ wait for the record to appear. Using the [/senders/add-domain](/transactional/api) endpoint, we will return a `verify_txt_key` which can be appended to `mandrill_verify. `in the same way.
...
You can also verify a domain with the Transactional API using the [`/senders/verify-domain`](/transactional/api) endpoint.
...
You can also view responses via the [Transactional API](/transactional/api) and webhooks in the `smtp_events` array.
...
You can also add dedicated IPs with the [Transactional API](/transactional/api).
...
3. Using the [`ips/start-warmup`](/transactional/api) endpoint in the Transactional API.
...
To remove a dedicated IP address, navigate to the [**Add Ons**](https://mandrillapp.com/account/billing/add-ons)** **page, locate the dedicated IP address you’d like to remove, and choose **Delete this IP** from the Actions menu. You can also remove a dedicated IP with the [Transactional API](/transactional/api). 
...
Setting up reverse DNS lets you connect a dedicated IP address to a custom domain name. You can manage this in the app, or with the [Transactional API](/transactional/api).
...
By default, Mailchimp Transactional creates a main pool for you, but you can create and manage pools within your account or with the [Transactional API](/transactional/api).
...
Once you’ve set up your pool, you can select it to apply globally in [**Sending Defaults**](https://mandrillapp.com/settings/sending-options). To change your dedicated pool on a per-message basis, `use `[`ip_pool`](/transactional/api) (API) or the [`X-MC-IPPool`](/transactional/docs/smtp-integration/#x-mc-ippool) header (SMTP).
