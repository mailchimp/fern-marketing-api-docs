...
First, let’s set up our webhook, which will be triggered when subscribers click a link in one of our plant emails. There are two ways to do this: using the Mailchimp Transactional app or [through the API](/transactional/api/webhooks/add). In this guide, we’ll walk through setting it up using the app. 
...
app.post("/", (req, res) => {
  ...
  mandrillEvents.forEach(event => {
    ...
    if (url === TARGET_LINK_URL) {
      ...
      // for more details, see /transactional/guides/send-first-email
...
If you’re using the [`webhooks/add`](/transactional/api/webhooks/add) method, the key will be returned in the response. You can also view and reset the key from the [**Webhooks**](https://mandrillapp.com/settings/webhooks)** **page in your account. To retrieve a webhook key via the Transactional API, use [`webhooks/info`](/transactional/api/webhooks/info) or [`webhooks/list`](/transactional/api/webhooks/list).
