...
<EndpointRequestSnippet endpoint="POST /3.0/lists/{list_id}/members" languages={["typescript"]}>
```typescript
{{snippet.imports}}
import { {{snippet.clientName}} } from "@mailchimp/mailchimp-marketing";

const client = new {{snippet.clientName}}({ apiKey: process.env.MAILCHIMP_API_KEY });
