...
There are two ways to set up our webhook: in the Mailchimp web app, or [through the API](/marketing/api). In this guide, we’ll walk through setting it up using the Mailchimp app.
...
You can also create list webhooks programmatically via the API. The create response includes a `signing_secret` field with the one-time plaintext value — the same "shown exactly once" contract applies. See the [Add Webhook API reference](https://mailchimp.com/developer/marketing/api/list-webhooks/add-webhook/) for request/response details.
...
- [Create Your First Audience](https://mailchimp.com/developer/guides/create-your-first-audience)
- [Webhooks API Reference](https://mailchimp.com/developer/api/marketing/list-webhooks)
