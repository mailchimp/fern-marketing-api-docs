...
<Note>
**Note**: The options payload includes an optional `occured_at` property (see more in [the API reference](/marketing/api)). You can set this to the date and time the event occurred in ISO 8601 format; if omitted, it will default to the current time. 
...
By default, the API will return the most recent 10 events. You can specify event count, offset, and [other options if needed](/marketing/api).
...
- [Events API reference](/marketing/api)
